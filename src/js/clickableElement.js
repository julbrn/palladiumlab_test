...
import {lerp, getCursorPos, calcWinsize, distance, getRandomFloat} from './utils';
...
window.addEventListener('mousemove', ev => cursorPos = getCursorPos(ev));

export default class ClickableElement extends EventEmitter {
  constructor(el) {
    super();
    // DOM elements
    // el: main button
  ...
  render() {
    ...
    const distanceMouseButton = distance(cursorPos.x + window.scrollX, cursorPos.y + window.scrollY, this.rect.left + this.rect.width / 2, this.rect.top + this.rect.height / 2);
    ...
    if (distanceMouseButton < this.distanceToTrigger) {
      ...
      x = (cursorPos.x + window.scrollX - (this.rect.left + this.rect.width / 2)) * .3;
      y = (cursorPos.y + window.scrollY - (this.rect.top + this.rect.height / 2)) * .3;
